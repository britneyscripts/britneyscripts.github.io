---
title: "Menocchio's Problem: On Building for Readers You Didn't Design For"
description: "How Carlo Ginzburg's evidential paradigm explains how AI agents read websites, and why Shopify's native MCP is changing the rules of e-commerce."
date: 2026-05-26T10:00:00-03:00
draft: false
tags: ["agentic-commerce", "mcp", "ucp", "rag", "product-management", "research"]
author: "Bê Acosta"
---

### The Unintended Reader

When I was in the University studying research methodology, the professor indicated to read a book called *The Cheese and the Worms*. This is a book written by Carlo Ginzburg. When I started reading the book, I didn't understand why I had to read about cheese, the inquisition, and a 16th-century Italian miller called Menocchio, but this book is my first friend when I lose track of something. 

Menocchio, as I said, was a miller, and he started to read books written for other audiences. Because of that, reading those books with a different lens, he created his own cosmology that compared the creation of the universe by God with the making of cheese: everything emerges from the Chaos.

The author wasn't interested in Menocchio himself but in the method: how you rebuild someone's thinking from fragments, indirect records, and signs. I know, you know, everyone knows it is like qualitative research—you need to gather all this disconnected data to build a storytelling and come up with a solution or a hypothesis. Ginzburg calls it an *evidential paradigm*. I bet you know a famous character that uses this method: Sherlock Holmes.

But what has Menocchio to do with all this? We can use this book under three perspectives: product discovery, observation methodology, and evidential paradigm. AI agents, like Menocchio, are kind of unintended website readers. They scan your page for fragments, piecing them together to build a coherent story about your product. And when the fragments are not there, everything emerges from chaos like a hallucination.

Hold this in your mind because today I'm going to talk about the first stage of my research: **Extracting Data**. I won't separate \"understanding business\" and \"extract data\" for this article, because they complement each other in this first stage.

---

### Starting Point: How Google Reads a Page

The first thing I did was thinking about the products I could use for my research. Just a quick reminder here: I'm focusing on how an agent (Gemini, ChatGPT, and so on) reads a PDP (product details page). This encompasses several dimensions: information architecture, schema completeness, readable content, performance, and user experience. 

Google Shopping data is on my radar too, but I'm not including it in this research iteration. And obviously, how Google Search reads a PDP matters here, though I'm not taking the traditional SEO approach. Domain authority, internal links, relevance scores—that's not what this is about.
...
Then I needed to add MCP and A2A protocols to my research framework. Why? Because Shopify just changed the rules. 

In December 2025, Shopify shipped native MCP support in its Winter '26 Edition. On March 24, 2026, Agentic Storefronts went live by default: 5.6 million Shopify stores became automatically discoverable inside ChatGPT, Microsoft Copilot, Google AI Mode, and the Gemini app. And on March 3, Shopify and Google jointly announced the Universal Commerce Protocol (UCP), an open standard that lets AI agents complete real purchases across platforms. Shopify was invited by Google to participate in this UCP development. 

I believe they invited Shopify over Adobe Commerce or other platforms because, let’s be honest here, Google is working hard to implement and launch UCP in the American market first. Shopify holds 30% of the US e-commerce market concentrated in DTC and mid-market, and if you’re launching a new product you need to be where the stores already are.
...
For my first test, I chose the *iPhone 16 Pro 128GB Titânio Preto* (yes, in Portuguese) and 5 platforms where this product is available in Brazil: Apple Brasil, iPlace, Mercado Livre, Amazon Brasil, and Fast Shop plus 6 more products (2 in the electronics category and 3 in skincare). This split was deliberate. I needed to understand how agents handle two fundamentally different types of entities.
...
But whether retinol is right for you depends on your skin type, your sensitivity, your routine. The agent needs both the product data and the user context to make a correct recommendation. Without `skin_type` declared in Schema as `additionalProperty`, it either infers or ignores, and inference here has real consequences. 

A wrong shade recommendation is an inconvenience. A wrong ingredient recommendation for sensitive skin is a return, a bad review, or worse. Let me show you what those fields look like:
...
* **RAM** &rarr; 8GB, 16GB
* **Storage** &rarr; 256GB SSD
* **Processor** &rarr; Apple M4, Snapdragon 8 Gen 3
* **Resolution** &rarr; 6.7\" AMOLED 120Hz
* **Battery** &rarr; 5000mAh
* **Connectivity** &rarr; 5G, Wi-Fi 6E, Bluetooth 5.3
...
One last note on extraction logic before we get to the data. The crawler script is the same for both categories: CrUX, PageSpeed, Schema.org presence, field completeness. That does not change. What changes is the list of expected fields per category. For electronics, I am checking for technical specs: processor, storage, RAM, connectivity. For skincare, I am checking for `additionalProperty` fields: skin type, ingredients, volume, SPF. 

Same infrastructure audit, different field map. Maybe in a future article we could go deeper on category page templates in e-commerce platforms like Shopify, VTEX, and Adobe Commerce.
...
The next thing to decide was: what is the better approach to collect data from a website: an agent or a simple crawler? Let's be honest here, I just needed to push the data for a test, verify that the API key is working, and get all the data I need. Just that, without overengineering the solution.
...
*(My Software Engineering degree is finally justifying its existence. It’s a joke, don’t take me wrong!)*
...
#### 3. Auditability
Here is where it gets interesting for academic research specifically. A crawler is explicit by design. Every extraction decision lives in the code: *\"field X expected, field X absent, log error.\"* You can read the script and understand exactly what was collected and how. 

An agent introduces a layer of interpretation that lives inside the model. Even with a well-structured skill (essentially an agent with pre-defined behavior), the decision about what a change means still passes through the LLM. That is not auditable in the same way that `if field_missing: log_error()` is auditable. A thesis reviewer can question results generated by an agent in ways they simply cannot question a Python script published on GitHub. The code is the methodology.
...
I use a crawler to collect because I need to account for every data point in my thesis defense. I use an agent to interpret because that is where flexibility beats precision.
...
* [Shopify MCP Explained](https://www.charle.co.uk/articles/shopify-mcp-explained/)
* [Shopify Winter '26 Edition - Model Context Protocol](https://ecommercefastlane.com/shopify-mcp-model-context-protocol)
* [Google Blog: Agentic Commerce AI Tools Protocol](https://blog.google/products/ads-commerce/agentic-commerce-ai-tools-protocol-retailers-platforms/)
* [Shopify Press: AI Commerce at Scale](https://www.shopify.com/news/ai-commerce-at-scale)
* [E-commerce Platform US Market Share](https://www.mobiloud.com/blog/ecommerce-platform-market-share-usa)
